...
void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y)
{
	DrawPattern(d, x, y, false, CRGB::Black);
}

void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y, CRGB ColTransparent) {
	DrawPattern(d, x, y, true, ColTransparent);
}

void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y, bool transparency, CRGB ColTransparent) {
	DrawPattern(d, x, y, transparency, ColTransparent, false, false);
}

void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y, bool transparency, CRGB ColTransparent, bool flipHoriz, bool flipVert){
	DrawPattern(d, x, y, transparency, ColTransparent, ROT_NONE, flipHoriz, flipVert);
}

void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y, bool transparency, CRGB ColTransparent, RotationType_t rotate) {
	DrawPattern(d, x, y, transparency, ColTransparent, rotate, false, false);
}

void cLEDMatrixBase::DrawPattern(iDrawable *d, int16_t x, int16_t y, bool transparency, CRGB ColTransparent, RotationType_t rotate, bool flipHoriz, bool flipVert) {
	int16_t src_x, src_y;
	int16_t tgt_x, tgt_y;

	for (int16_t dy=0; dy<d->Height(); dy++) {
		for (int16_t dx=0; dx<d->Width(); dx++) {
			if (flipHoriz) {
				src_x = d->Width()-1-dx;
			} else {
				src_x = dx;
			...
			CRGB src = d->Pixel(src_x, src_y);
			if (!transparency || ColTransparent != src) {
				switch (rotate) {
				case ROT_NONE:
					tgt_x = x+dx;
					tgt_y = y+dy;
					break;
				case ROT_90_RIGHT:
					tgt_x = x + d->Height()-1-dy;
					tgt_y = y + dx;
					break;
				case ROT_90_LEFT:
					tgt_x = x + dy;
					tgt_y = y + d->Width() - 1 - dx;
					break;
				case ROT_180:
					tgt_x = x + d->Width() - 1 - dx;
					tgt_y = y + d->Height() - 1 - dy;
					break;
				default:
					//should not happen
					break;
				}
				DrawPixel(tgt_x, tgt_y, src);
			}
		}
	}
}
